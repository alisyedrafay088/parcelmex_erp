...
import { parcelsApi, type Parcel, type AddressVerificationStatus } from "../api/parcels";
...
export function AddressVerification() {
  ...
  async function load() {
    if (!token) return;
    try {
      const [p, c] = await Promise.all([parcelsApi.list(token), clientsApi.list(token)]);
      setParcels(p);
      setClients(c);
      setLoading(false);
    } catch (err) {
  ...
  const filteredParcels = useMemo(() => {
    ...
    return list;
  }, [parcels, clients, filter, search]);

  const selectedParcel = parcels.find((p) => p.id === selectedId) ?? null;
  const selectedClient = selectedParcel ? clients.find((c) => c.id === selectedParcel.client_id) ?? null : null;
  ...
  // Automatically check the address whenever it changes -- either because a
  // different parcel was selected, or because the address was edited by hand.
  // Typing gets a short debounce so it doesn't fire a geocode on every keystroke.
  useEffect(() => {
  ...
  async function handleSetStatus(status: AddressVerificationStatus) {
    ...
    setError(null);
    try {
      const updated = await parcelsApi.verifyAddress(token, selectedParcel.id, {
        status,
        lat: checkResult?.lat,
        lng: checkResult?.lng,
        ...
        <div className="airway-layout">
          <div className="airway-picker">
            ...
            <div className="login-input-wrap airway-search-wrap">
              <Search size={16} />
              <input
                placeholder="Search by tracking ID, client or address..."
                value={search}
              ...
              <>
                <div className="av-detail-header">
                  <div>
                    ...
                    <span className="fleet-muted"> · {selectedClient?.name ?? `#${selectedParcel.client_id}`}</span>
                  </div>
                ...
                <label htmlFor="av-address" className="av-label">
                  Delivery Address
                </label>
                <textarea
                  id="av-address"
                ...
                <div className="av-action-row">
                  <button type="button" className="fleet-add-button fleet-add-button-secondary" onClick={handleCheck} disabled={checking}>
