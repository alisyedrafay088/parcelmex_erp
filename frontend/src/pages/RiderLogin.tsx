...
export function RiderLogin() {
  ...
  if (!loading && token && user?.role === "rider") return <Navigate to="/rider" replace />;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (loggedInUser.role !== "rider") {
        logout();
        setError("This isn't a rider account. Please use the admin or customer login instead.");
        return;
      }
  ...
  return (
    <div className="portal-login-page">
      <form className="portal-login-card" onSubmit={handleSubmit}>
        <Logo size="md" />
        <h2 className="portal-login-heading">Rider Login</h2>
        <p className="portal-login-subtitle">Sign in to see your assigned deliveries.</p>
