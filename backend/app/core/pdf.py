...
def generate_invoice_pdf(invoice: Invoice, client: Client) -> bytes:
    ...
    c.drawString(20 * mm, height - 16 * mm, "PARCEL MEX.")
    ...
    c.drawCentredString(width / 2, 15 * mm, "Thank you for partnering with Parcel Mex.")
...
def generate_expense_report_pdf(expenses: list[Expense], start: date | None, end: date | None) -> bytes:
    ...
    def draw_header():
        ...
        c.drawString(margin, height - 12 * mm, "PARCEL MEX.")
        c.setFillColor(ORANGE)
    ...
    c.drawCentredString(width / 2, margin / 2, f"Generated by Parcel Mex on {date.today().strftime('%d %b %Y')}")
